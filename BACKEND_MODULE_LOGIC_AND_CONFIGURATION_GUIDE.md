...
`ContractOnboardingService` manages required submissions and signer state. Contract creation initializes `contract_requirements` and `contract_signers`. Tenants/staff submit documents; authorized staff review them, control guardian-required state, and update signer records. Activating a contract is protected by verified-email and completion requirements.
